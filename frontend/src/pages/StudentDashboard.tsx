        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <Card className="mt-6">
            ...
            <div className="space-y-3">
              {counselors.length === 0 ? (
                <p className="text-xs text-text-muted text-center py-2">No counselors available.</p>
              ) : (
                counselors.map(c => (
                  <div
                    key={c.id}
          ...
          <Card className="h-full min-h-[400px]">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex bg-canvas rounded-lg p-1">
                <button 
                  onClick={() => setRange('7D')}
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${range === '7D' ? 'bg-surface shadow text-text-main' : 'text-text-muted hover:text-text-main'}`}
                >
                  7D
                </button>
                <button 
                  onClick={() => setRange('30D')}
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${range === '30D' ? 'bg-surface shadow text-text-main' : 'text-text-muted hover:text-text-main'}`}
                >
                  30D
                </button>
              </div>
